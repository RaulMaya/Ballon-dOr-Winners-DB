  ...
  const newClub = await inquirer.prompt([
    {
      ...
      validate: (answer) => {
        const ansClub = answer.split(" ");
        for (let i = 0; i < ansClub.length; i++) {
          ansClub[i] = ansClub[i].charAt(0).toUpperCase() + ansClub[i].slice(1);
        }
        const addingClub = ansClub.join(" ");
        if (clubList.includes(addingClub)) {
          return "This is club is already in our database.";
        }
        return true;
      },
    },
